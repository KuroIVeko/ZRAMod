...
- **空闲页二次压缩（可选，默认关闭）**：内核开启了 `CONFIG_ZRAM_MULTI_COMP` 时，可以在 WebUI 里开启 recompress。重建 zram 时（`reset` 之后、写 `disksize` 之前）注册次级算法；开机完成后由后台守护进程按间隔执行：先 `echo all > idle` 标记，下一轮对仍然空闲的页执行 `type=idle` 重压，以 `nice 19` 运行。次级算法只能在 zram 初始化前注册，所以开启后需要重新应用一次或重启才生效。注册失败只记日志，不影响 zram 本身，也不会阻塞开机。守护进程状态和每轮节省的空间会显示在 WebUI 和日志里（`/data/adb/zramod/recomp_status.conf`）。
