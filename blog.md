# ZRAMod：把我每次开机手动敲的 zram 命令，做成一个 KernelSU 模块

## 起因

我一直习惯自己调 zram：换个压缩算法、把容量开大一点、把 `swappiness` 和 `watermark_scale_factor` 调到自己觉得舒服的数值。问题是，这些改动都是运行时状态，重启就没了。于是很长一段时间里，我的日常是这样的：开机、进终端、`su`，然后敲一遍这个：
